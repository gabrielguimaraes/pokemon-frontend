...
#   -u, --user USER     SSH username (default: testrigor) [stored as SSH_USER]
...
SSH_USER="${DEPLOY_USER:-testrigor}"
...
if [[ "${BACKEND_ONLY}" == false ]]; then
    ...
    rsync ${RSYNC_OPTS} "${SSH_CMD}" --progress --delete \
        "${LOCAL_FRONTEND_BUILD}" "${SSH_USER}@${SERVER}:${REMOTE_FRONTEND_PATH}"
...
if [[ "${FRONTEND_ONLY}" == false ]]; then
    ...
    rsync ${RSYNC_OPTS} "${SSH_CMD}" --progress \
        "${LOCAL_BACKEND_BUNDLE}" "${SSH_USER}@${SERVER}:${REMOTE_BACKEND_PATH}"
...
${SSH_CMD} "${SSH_USER}@${SERVER}" << ENDSSH
    set -e
